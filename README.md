...
```text
.
├── main.py                                     # 程序入口：多账号调度、投币与推送
├── bilibili.py                                 # B 站任务 API 封装
├── push.py                                     # 报告格式化与 PushPlus 推送
├── requirements.txt                            # Python 依赖声明
├── .github/workflows/Bilibili_DailyCheckin.yml  # 定时与手动触发流程
└── console/
...
所有配置只涉及 2 个 Secret 与 4 个 Variable，名称、默认值均与代码一致。配置方式二选一，效果完全相同：
...
3. **任务配置**：填写 Cookie / PushPlus Token，勾选任务、投币数 / 来源 / 点赞后点保存配置，字段含义见「配置参考」
   - Cookie / PushPlus Token 写入 Secrets，留空 = 不修改
   - 任务与投币设置写入 Variables
